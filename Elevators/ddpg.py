...
from priority_buffer import PriorityReplayBuffer
from networks import hard_update,hidden_init
...
class DDPG():
    ...
    def act(self, state):
        ...
        # renorm
        for el in range(action.shape[1]):
            if np.min(action[:,el,:]) < 0:
                action[:,el,:] += np.absolute(np.min(action[:,el,:])) * 2
            action[:,el,:]  = action[:,el,:] / np.sum(action[:,el,:])
        self.actor.train()
        return action
